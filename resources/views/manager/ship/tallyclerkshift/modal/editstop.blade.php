<div class="modal" id="exampleEditStop{{$item->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content">
        ...
        <form method="POST" action="{{route('manager-stop.update',$item->id)}}">
          @csrf
         @method('PATCH')
        ...
        <div class="form-group">
          ...
          <textarea type="time" name="reason" class="form-control" placeholder="{{__('text.reason')}}" required readonly>{{$item->reason}}</textarea>
        </div>
