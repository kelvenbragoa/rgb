...
class TallyClerkShiftShipController extends Controller
{
    ...
    public function index( $tallclerkshiftship_id, $ship_id){
        ...
        $stops = StopRecord::where('tally_clerk_ship_id', $tallclerkshiftship_id)->get();
        $shiftship = ShiftShip::find($tallyclerkshiftship->shiftship->id);
        $tallyclerk= TallyClerk::find($tallyclerkshiftship->tally_clerk_id);

        

        $stops_time = StopRecord::where('tally_clerk_ship_id', $tallclerkshiftship_id)->where('status',1)->get();
        ...
        return view('admin.ship.tallyclerkshift.show',compact('ship','tallyclerkshiftship','tallybook','stops','shiftship','tallyclerk','time_total'));
